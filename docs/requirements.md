...
7. `./scripts/verify_docker_stack.sh`로 Isaac Sim 5.1.0 / Isaac Lab 2.3.2, 기본 `rsl_rl`, `isaacsim.core.prims` import 확인
...
참고:
- 기본 빌드는 `rsl_rl` 기준이다.
- `verify_docker_stack.sh`는 headless `SimulationApp`을 띄운 뒤 `rsl-rl-lib`, `onnxscript`, `isaaclab_rl.rsl_rl` import까지 확인한다.
