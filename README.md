...
회사망에서 GitHub outbound가 막혀 있으면 `rl_games` 설치는 기본적으로 건너뛴다. 현재 기본 알고리즘은 `rsl_rl_ppo`라서 기본 학습 경로에는 영향이 없다. `rl_games`가 정말 필요할 때만 아래처럼 켠다. 이 경로는 `rl_games`를 별도 clone한 뒤 `--no-build-isolation`으로 설치한다.
